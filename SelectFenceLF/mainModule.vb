    ...
    Else
        ...
        Else
           ...
           If selectedShapeOK Then
                ...
                End If
           Else
                Message.Show ("No valid shape defined")
           End If
        End If
    End If
...
Public Function createFenceFromShapeElement(ByRef sElement) As Boolean
    ...
    Dim lastView As View
     lastView = CommandState.lastView
...
Sub getStatsConductor()
    ...
    Message.Show("Max = " & maxLength & "Min Length : " & minLength)
...
Public Sub GetPhaseAllocation(A_count As Integer, B_count As Integer, C_count As Integer)
    ...
    'If Not (ActiveModelReference.AnyElementsSelected) Then
    '    Message.Show ("Phase does'nt exist")
    'End If
...
Sub selectOnlyPhases(ByVal aPhase As Boolean, bPhase As Boolean, cPhase As Boolean)
    ...
    If aPhase And bPhase And cPhase Then
        Message.Show("Phase A :" & phaseA_count & vbCrLf &
                "Phase B :" & phaseB_count & vbCrLf &
                "Phase C :" & phaseC_count & vbCrLf)
    End If

    If Not (ActiveModelReference.AnyElementsSelected) Then
        Message.Show("Phase doesnt exist")
    End If
...
Sub PopulateElementLists(ByRef oFenceEnum As ElementEnumerator, houseTextList, TrfrShapesList, TRFRList, AirdacList, LVpolesList, MVpolesList, MVSharingPolesListLVConductorList, MVConductorList, LVStaysAndAnchorsList, MVStaysAndAnchorsList, elseElements As Collection)
    ActiveModelReference.UnselectAllElements
    oFenceEnum = ActiveDesignFile.Fence.GetContents
    Dim oElem As Element
    Do While oFenceEnum.MoveNext
        oElem = oFenceEnum.Current
...
Sub getConductorStats()
    ...
    Message.Show("Max = " & maxLength & vbCrLf & "Min Length : " & minLength & vbCrLf & "Average : " & aveLength & vbCrLf & " Total length : " & totalLength)
...
Sub stats(ByVal LVStats As String)
    ...
    TRFZone = New clsTrfZone
    TRFRZone = New clsTrfrZone
    txtLvl55 = New Collection
    txtLvl56 = New Collection
    txtLvl57 = New Collection
    txtLvl58 = New Collection

    LVpoles = New Collection
    mvpoles = New Collection
    mvlvpoles = New Collection
    LVStays = New Collection
    MVStays = New Collection
    ...
    ABC_1PH = New Collection
    ABC_2PH = New Collection
    ARVIDAL_35 = New Collection
    LVStruts = New Collection
    LVFlyingStays = New Collection
    OAK = New Collection
    txtLvl57 = New Collection
    ServiceCables = New Collection
    length1 = 0
    length2 = 0
        ...
        Select Case oElem.type
            Case msdElementTypeLine
                TRFRZone.Conductors.Add oElem
                    If (oElem.AsLineElement.LineStyle.Name = "ABC35-R") Or
                        (oElem.AsLineElement.LineStyle.Name = "ABC35-W") Or
                ...
                ElseIf (oElem.AsLineElement.LineStyle.Name = "ABC35-RW") Or
                        (oElem.AsLineElement.LineStyle.Name = "ABC35-WB") Or
                ...
                ElseIf (oElem.AsLineElement.LineStyle.Name = "35") Then
                    ARVIDAL_35.Add oElem
                        oElem.Redraw(MsdDrawingModeHilite)
                    ActiveModelReference.SelectElement(oElem)
                ElseIf (oElem.AsLineElement.LineStyle.Name = "4mm AD A") Or
                        (oElem.AsLineElement.LineStyle.Name = "4mm AD B") Or
                        (oElem.AsLineElement.LineStyle.Name = "4mm AD") Or
    ...
    gABC_2PH = ABC_2PH
    gLVPoles = LVpoles
    gMVLVPoles = mvlvpoles
    gLVStays = LVStays
    gLVStruts = LVStruts
    gLVFlyingStays = LVFlyingStays
    gServiceCables = ServiceCables
    gTxtLvl57 = txtLvl57
...
Function ProcessText(ByVal textCol As Collection) As String
    ...
    ABC_1145 = New Collection
    ABC_1146 = New Collection
    ABC_1147 = New Collection
    ABC_1148 = New Collection
    ABC_1149 = New Collection
    ABC_1151 = New Collection
    ABC_1152 = New Collection
    ABC_1153 = New Collection
    ABC_1154 = New Collection
    ABC_1155 = New Collection
    ABC_1156 = New Collection
    ABC_1157 = New Collection
    ABC_1158 = New Collection
    ABC_1159 = New Collection
    ...
    For Each oelm In textCol
        ...
        End Select
        '1145
        oText = "LV Structures '1145' : " & ABC_1145.count & vbCrLf +
                "LV Structures '1146' : " & ABC_1146.count & vbCrLf +
                "LV Structures '1147' : " & ABC_1147.count & vbCrLf +
                "LV Structures '1148' : " & ABC_1148.count & vbCrLf +
                "LV Structures '1149' : " & ABC_1149.count & vbCrLf +
                "LV Structures '1151' : " & ABC_1151.count & vbCrLf +
                "LV Structures '1152' : " & ABC_1152.count & vbCrLf +
                "LV Structures '1153' : " & ABC_1153.count & vbCrLf +
                "LV Structures '1154' : " & ABC_1154.count & vbCrLf +
                "LV Structures '1155' : " & ABC_1155.count & vbCrLf +
                "LV Structures '1156' : " & ABC_1156.count & vbCrLf +
                "LV Structures '1157' : " & ABC_1157.count & vbCrLf +
                "LV Structures '1158' : " & ABC_1158.count & vbCrLf +
...
Function getPoleTopBoxes(ByVal LVCol As Collection, ByVal MVLVCol As Collection) As String
    ...
    poleTopBoxes = New Collection
    For Each ppole In LVCol
        'Dim newBox As clsPoleTopBox
        ' newBox = New clsPoleTopBox
...
Sub textfontchange()
    ...
    Dim oFont As Font
    tEle = CreateTextElement1(Nothing, "hello", Point3dZero, Matrix3dIdentity)
    oFont = ActiveDesignFile.Fonts("Verdana")
    tEle.TextStyle.Font = oFont
...
Public Sub ChangeCell104(ECell As CellElement) ', TrfrName As String, Rating As String)
    ...
    While oEE1.MoveNext
        ...
        ElseIf AElement1.IsTextElement Then
            ...
            oFont = ActiveDesignFile.Fonts("ISO")
...
Sub SelectAllZones()
    ...
    point.Y = startPoint.Y
    point.Z = startPoint.Z
    point2.X = point.X + 0.00000000000068212103
...
Function IniytializePrefixStrings(low As Long, high As Long, prefix As String) As String()
    ...
    'Dim Val As Long
    If low > high Then
        Message.Show("Size of array cannot be negative"
        Exit Function
    End If
...
Public Function ProcessFence(ee As ElementEnumerator) As Boolean
    ...
    Exit Function
ERROR_HANDLER:
    Message.Show("ProcessFence Error: " & CStr(Err.Number) & Err.Description
End Function
...
Sub GetAllStats(outputTxt As String)
    ...
    trfZoneEnum = ActiveModelReference.GetSelectedElements
    Dim elArray() As Element
    ...
    For i = iStart To iEnd
        ...
        GetTrfCell trfElem
        'selectNormalCells "TRFRS"
        ' trfEnum = ActiveModelReference.GetSelectedElements
        'trfEnum.Reset
        Dim ix As Long
        Dim iStartx As Long
        Dim iEndx As Long
